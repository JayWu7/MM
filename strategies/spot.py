...
class Spot(StrategyBase):
    ...
    def compute_current_bins(self, current_price: float, cur_inventory_amount: float, cur_quote_amount: float) -> dict:
        '''
            Compute bid and ask bins symmetrically around the current price using a uniform (CLMM-style) distribution.
            Order sizes are adjusted based on inventory imbalance using linear scaling near rebalance thresholds.
        '''
