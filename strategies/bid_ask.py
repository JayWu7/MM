...
class BidAsk(StrategyBase):
    def __init__(self, 
                 ...
                 quote_rb_iqv_ratio: float = -0.3) -> None:
        ...
        self.strategy_name = 'Bid-Ask Mode Market-Maker Strategy'


    def compute_current_bins(self, current_price: float, cur_inventory_amount: float, cur_quote_amount: float, decay_rate=0.95) -> dict:
        '''
            Compute the current bid and ask bin prices and sizes using a Tailed Liquidity Distribution strategy.

            This strategy places smaller order sizes near the current mid-price and increases the order size 
            as the distance from the mid-price increases. It is ideal for volatile markets, aiming to capture 
            larger profits from large price swings.

            Parameters
            ----------
            decay_rate : float, optional (default=0.95)
                Geometric decay factor used to compute bin sizes. 
                The formula for size at bin index `i` (distance from mid) is:
                    size_i = max_order_size * (1 - decay_rate ** (i + 1))
                Sizes increase as we move further from the mid price.
            
            Calculation Steps:
            ------------------
            1. Define total number of bid and ask bins: self.bid_bin_nums and self.ask_bin_nums.
            2. For each bin, calculate distance from the center.
            3. Assign a weight using inverse curve formula: 
                weight[i] = 1 / (decay_ratio^distance_from_center + ε)
            4. Normalize all weights so that they sum up to the total capital available on each side.
            5. Compute order sizes by multiplying each normalized weight by the total capital available.
        '''
