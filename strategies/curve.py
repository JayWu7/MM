...
class Curve(StrategyBase):
    ...
    def compute_current_bins(self, current_price: float, cur_inventory_amount: float, cur_quote_amount: float, decay_rate=0.95) -> dict:
        '''
            Compute bid and ask bins with a Curve-style distribution.
            Orders are concentrated around the mid price: larger size at the center,
            smaller size further away. Ideal for stable pairs with low volatility.

            the order size for each price bin using exponential decay.

            Formula Explanation:
            --------------------
            - Each bin's order size decays exponentially from the center (or from the first bin).
            - Decay factor: decay_ratio^i, where i is the bin index (i = 0 to bin_nums - 1)
            - Total normalized weight:
                sum = decay_ratio^0 + decay_ratio^1 + ... + decay_ratio^(bin_nums - 1)
                    = (1 - decay_ratio^bin_nums) / (1 - decay_ratio)
            - Max size = total_size / sum
            - Size for bin i = max_size * decay_ratio^i

            Parameters:
            -----------

            decay_rate : float
                Decay factor per bin, must be in (0, 1). A smaller decay_ratio results in a steeper drop-off.
        '''
